// ============================================================
// Testbench  : tb_residue_comparison
// Project    : Self-Healing Digital Circuit
// Description:
//   Direct side-by-side accuracy comparison of adaptive
//   architectures with and without residue-based quarantine.
//
//   Architectures:
//   1. Adaptive (No Residue) — voter-only quarantine
//   2. Adaptive (With Residue) — residue-based quarantine
//
//   The testbench runs identical fault injection scenarios
//   on both architectures and reports relative accuracy
//   improvement from residue-based fault localization.
// ============================================================

`timescale 1ns/1ps

module tb_residue_comparison;

    parameter CLK_HALF  = 5;
    parameter N_RANDOM  = 10_000;
    parameter N_CORNER  = 500;
    parameter N_WARMUP  = 512;
    ...
    initial begin
        $dumpfile("tb_residue_comparison.vcd");
        $dumpvars(0, tb_residue_comparison);
        ...
        $display("");
        $display("====================================================================");
        $display("  RESIDUE-BASED FAULT LOCALIZATION IMPACT ANALYSIS");
        $display("  Comparison: Adaptive (No Residue) vs Adaptive (With Residue)");
        $display("  Vectors per scenario: %0d   Warm-up cycles: %0d", N_RANDOM, N_WARMUP);
        $display("====================================================================");
        ...
        do_reset;
        reset_counters;
        fi0_en = 1'b1; fi0_type = 2'b00; fi0_mask = 8'hFF;  // stuck-at-0
        fi1_en = 1'b0; fi2_en = 1'b0; fi3_en = 1'b0; fi4_en = 1'b0;
        do_warmup(N_WARMUP);
        for (i = 0; i < N_RANDOM; i = i + 1) begin
            lfsr_step; a = lfsr[7:0];
        ...
        // ==================================================================
        // S4 — SINGLE FAULT: ALU0 STUCK-AT-1
        // ==================================================================
        do_reset;
        reset_counters;
        fi0_en = 1'b1; fi0_type = 2'b01; fi0_mask = 8'hFF;  // stuck-at-1
        fi1_en = 1'b0; fi2_en = 1'b0; fi3_en = 1'b0; fi4_en = 1'b0;
        do_warmup(N_WARMUP);
        for (i = 0; i < N_RANDOM; i = i + 1) begin
            lfsr_step; a = lfsr[7:0];
        ...
        $display("| S4  ALU0 stuck-at-1 (single fault)                | %6.2f%%     | %6.2f%%      | %7.2f%%     |",
            100.0 * no_res_ok / no_res_tot, 100.0 * with_res_ok / with_res_tot,
            (100.0 * with_res_ok / with_res_tot) - (100.0 * no_res_ok / no_res_tot));
        merge_grand;
